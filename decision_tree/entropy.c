...
double Entropy(int num_pos, int num_neg) {
  ...
  /* Insert entropy calculation here */

  double total_sample = num_pos + num_neg;
  double p_plus = ((double)num_pos) / total_sample;
  double p_minus = ((double)num_neg) / total_sample;

  entropy = -1 * p_plus * LogBase2(p_plus) - p_minus * LogBase2(p_minus);
  return entropy;
}
